...
if(isset($_POST['save'])){
    ...
    if($ret > 0){ 
        ...
        $query=mysqli_query($conn,"insert into tblclassarms(CourseCode,classId,CourseName,AssignedTo,isAssigned) value('$coursecode','$classId','$coursename','$assignid','1')");
      ...
      $row=mysqli_fetch_array($query);
        ...
        if(isset($_POST['update'])){
          ...
          $assignid=$_POST['teacher'];
            ...
            $query=mysqli_query($conn,"update tblclassarms set CourseName='$coursename', AssignedTo='$assignid' where classId='$classId' AND CourseCode='$coursecode'");
                  ...
                  <form method="post">
                    <div class="form-group row mb-3">
                        ...
                        if ($num > 0){
                          ...
                          echo'<option value="'.$rows['Id'].'" >'.$rows['Program'].'-'.$rows['Year(Batch)'].'-'.$rows['section'].'</option>';
                              }
                                  echo '</select>';
                              }
                            ?>  
                      ...
                      <input type="text" class="form-control" name="coursecode" value="<?php echo $row['CourseCode'];?>"  placeholder="Course Code">
                        </div>
                    </div>
                    <div class="form-group row mb-3">
                        <div class="col-xl-6">
                        ...
                        $num = $result->num_rows;		
                        if ($num > 0){
                          echo ' <select required name="teacher" class="form-control mb-3">';
              ...
              <div class="card mb-4">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                    ...
                    <tbody>
                    <?php
                      $query = "SELECT tblclassarms.CourseName,tblclassarms.isAssigned,tblclassteacher.firstName,tblclassteacher.lastName FROM tblclassarms
                      INNER JOIN tblclassteacher ON tblclassarms.AssignedTo=tblclassteacher.Id";
                      $rs = $conn->query($query);
                      $num = $rs->num_rows;
                      $sn=0;
                      $status="";
